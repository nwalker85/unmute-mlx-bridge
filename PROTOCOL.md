...
# config.json for the MLX repo variant, not re-specified by this bridge.
```
...
Note `stt.toml` and `tts.toml` reference `kyutai/stt-1b-en_fr-candle` (a
Candle/CPU-GPU checkpoint layout), not the `-mlx` suffixed repo. This bridge
uses `kyutai/stt-1b-en_fr-mlx` and `kyutai/tts-1.6b-en_fr` (default, no `-mlx`
suffix needed — its `moshi_name`/`mimi_name`/`tokenizer_name` fields are the
same PyTorch-format safetensors `moshi_mlx.load_pytorch_weights` reads
directly) — these are the repos Kyutai publishes specifically for MLX inference
(`kyutai/stt-1b-en_fr-mlx/config.json`'s `model_type: "stt"` +
`mimi_name`/`tokenizer_name` fields drive `SttModelBundle.load` directly; no
TOML translation layer needed).
