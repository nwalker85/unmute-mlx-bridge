"""Shared HTTP operations surface: `/healthz`, `/readyz`, `/metrics`, and auth.
...
def build_process_request(
    ...
    Handles `/healthz`, `/readyz`, `/metrics` directly, and pre-upgrade-rejects
    unauthorized requests to `protocol_path` with HTTP 401 — matching real
    `moshi-server`, which checks `authorized_ids` before completing the WebSocket
    handshake (`main.rs::streaming_t`), not after.
    """
