...
> tested. Not yet run against the stock Unmute orchestrator or in a full
> microphone-to-speaker canary; see **What's proven / What's not**, below.
...
- Both servers implement the full session lifecycle against that protocol over
  ...
  platform).
- Real MLX STT inference: real `kyutai/stt-1b-en_fr-mlx` weights, a real
...
- The stock Unmute backend has **not** been pointed at these servers. The
  protocol conformance is proven against a from-source reading of Unmute's
  client code, not against Unmute's process actually running.
...
| `STT_HF_REPO` | STT | Default `kyutai/stt-1b-en_fr-mlx` |
...
- `tests/hardware/test_stt_hardware.py` — synthesizes a known sentence with
  macOS `say`, transcribes it with real `kyutai/stt-1b-en_fr-mlx` inference via
  `SttSession`, and asserts the expected words appear with monotonically
  increasing timestamps. Also runs two sequential sessions on the same process
  to confirm cache/state does not leak between connections.
